...
function adjacencyListToCells(graph: Graph) {
...
function makeTransition(id: number, label: string) {
...
function makeLink(parentElementLabel: string, childElementLabel: string) {
...
function makeElement(label: string, marking: number) {
    const maxLineLength = label.split('\n').reduce((max, l) => {
        return Math.max(l.length, max);
    }, 0);
...
class Graph {
    ...
    transitions: {
        label: string,
        inputArcs: [string, number][],
        outputArcs: [string, number][]
    }[] = [];
    marking: number[] = [];
    constructor(data: {
        places: string[], transitions: {
            label: string,
            inputArcs: [string, number][],
            outputArcs: [string, number][]
        }[]
    ...
    minM0(omega: string[]) {
        ...
        let result = minM0({
            B, BMinus, T, L: (t: number) => Larray[t]
        }, omega);
        return result;
    }
}
...
document.getElementById("minM0")!.onclick = () => {
    let ta: HTMLTextAreaElement = <HTMLTextAreaElement>document.getElementById("graphdata")!;
